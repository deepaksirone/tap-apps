        /* IFTTT Filter V 1.6  
         *** Heavy mods/adds to the SunCalc Library
        */
        /*
        ...
        */
        ...
        // Log a debugging string to the HTML page
        // *** THIS IS ONLY DESINGED TO WORK IN
        // *** THE TYPESCRIPT PLAYGROUND
        // *** https://www.typescriptlang.org/play/ 
        function log(str: any) {
            // Only uncomment in the TypeScript Playground
            //document.body.outerHTML = document.body.outerHTML + '<br>' + str;
        }
        ...
        var SunAngleIndex = 0,
            MorningEventNameIndex = 1,
            EveningEventNameIndex = 2,
            MorningEventTimeIndex = 3,
            EveningEventTimeIndex = 4;
        ...
        function getTimes(date: any, lat: any, lng: any) {
            ...
            var lw = rad * -lng,
                ...
                i, len, time: any[], Jset, Jrise;


            // NOTE: the next two sections are very different than the
            // original SunCalc Library.  Some things didn't translate
            // well from Javascript to TypeAcript    
            ...
            for (i = 0, len = times.length; i < len; i += 1) {
                ...
                TimeResults[i][MorningEventTimeIndex] = fromJulian(Jrise);
                TimeResults[i][EveningEventTimeIndex] = fromJulian(Jset);
            }

            var result = {
                solarNoon: fromJulian(Jnoon),
                nadir: fromJulian(Jnoon - 0.5),
                sunrise: TimeResults[0][MorningEventTimeIndex],
                sunset: TimeResults[0][EveningEventTimeIndex],
                sunriseEnd: TimeResults[1][MorningEventTimeIndex],
                sunsetStart: TimeResults[1][EveningEventTimeIndex],
                dawn: TimeResults[2][MorningEventTimeIndex],
                dusk: TimeResults[2][EveningEventTimeIndex],
                nauticalDawn: TimeResults[3][MorningEventTimeIndex],
                nauticalDusk: TimeResults[3][EveningEventTimeIndex],
                nightEnd: TimeResults[4][MorningEventTimeIndex],
                night: TimeResults[4][EveningEventTimeIndex],
                goldenHourEnd: TimeResults[5][MorningEventTimeIndex],
                goldenHour: TimeResults[5][EveningEventTimeIndex],
            };
        ...
        // remember, these only work in the 
        // TypeScript Playground
        log('solarNoon -- [' + time.solarNoon + ']')
        log('nadir -- [' + time.nadir + ']')
        log('Sunrise -- [' + time.sunrise + ']')
        log('Sunset -- [' + time.sunset + ']')
        log('sunriseEnd -- [' + time.sunriseEnd + ']')
        log('sunriseStart -- [' + time.sunsetStart + ']')
        log('dawn -- [' + time.dawn + ']')
        log('dusk -- [' + time.dusk + ']')
        log('nauticalDawn -- [' + time.nauticalDawn + ']')
        log('nauticalDusk -- [' + time.nauticalDusk + ']')
        log('nightEnd -- [' + time.nightEnd + ']')
        log('night -- [' + time.night + ']')
        log('goldenHourEnd -- [' + time.goldenHourEnd + ']')
        log('goldenHour -- [' + time.goldenHour + ']')
        ...
        */
        ...
        //Sunrise Logic
        // If we're past the delay time after sunrise 
        if (minAfterSunrise >= sunriseDelay) {
            // We're inside Trigger window, 
            // Trigger!!!!!   :-)
            log('Trigger!')
            ...
            // We're not yet in the trigger window, or it's before sunrise 
            // Don't trigger    :-(
            log('Do not Trigger!')
